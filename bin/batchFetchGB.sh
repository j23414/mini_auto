...
#	curl "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nuccore&amp;id=${QUERY};rettype=gb&amp;retmode=text" >> out.gb
#	sleep 1;
	NUM=1;
	QUERY="${IDS}";
	FILE=$((FILE+BATCH));
...
then
    ...
    [[ -f gb/${FILE}.gb ]] || sleep 1
    [[ -f gb/${FILE}.gb ]] || curl "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nuccore&amp;id=${QUERY};rettype=gb&amp;retmode=text" > gb/${FILE}.gb
#    curl "https://eutils.ncbi.nlm.nih.gov/entrez/eutils/efetch.fcgi?db=nuccore&amp;id=${QUERY};rettype=gb&ampretmode=text" >> out.gb
fi
